...
Average buy price is saved and restored correctly after restarting the application.
...
Realized P&L is calculated using the holding's stored average buy price.
Realized P&L is saved and restored across restarts.

Examples:

Sell specific qty at explicit price: /remove --type STOCK --ticker VOO --qty 0.5 --price 620
Sell specific qty at last /set price: /remove --type STOCK --ticker VOO --qty 0.5
Sell all at explicit price: /remove --type STOCK --ticker VOO --price 590
Sell all at last /set price: /remove --type STOCK --ticker VOO
...
A: Older save files are still supported. If they do not contain a separate average buy price, the saved price is used as the cost basis for compatibility.
